    ...
    public class BHUtilities
    {
        ...
        public static string ResourcePath
        {
            get
            {
                ...
                string[] beacon = AssetDatabase.FindAssets("\"BMT_Resources_Beacon\"");
                if (beacon == null || beacon.Length == 0)
                    throw new UnityException("BMT_Resources_Beacon not found");

                _resourcePath = AssetDatabase.GUIDToAssetPath(beacon[0]);

                int endPathIndex = _resourcePath.LastIndexOf("/");
                if (endPathIndex < _resourcePath.Length - 1)
                    _resourcePath = _resourcePath.Remove(endPathIndex + 1);
                return _resourcePath;
            }
        }
