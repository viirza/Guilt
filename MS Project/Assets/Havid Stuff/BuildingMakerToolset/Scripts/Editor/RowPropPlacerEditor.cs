...
namespace BuildingMakerToolset.PropPlacer
{
    ...
    public class RowPropPlacerEditor : Editor
    {
        ...
        bool gizmosEnabledByTool = false;
        ...
        bool _needPreviewMesh = false;
        bool NeedPreviewMesh
        {
            get
            {
                return _needPreviewMesh;
            }
            set
            {
                if (_needPreviewMesh == value)
                    return;
                _needPreviewMesh = value;
                if (_needPreviewMesh)
                {
                    gizmosEnabledByTool = SceneView.lastActiveSceneView.drawGizmos == false;
                    SceneView.lastActiveSceneView.drawGizmos = true;
                }
                else if(gizmosEnabledByTool)
                {
        ...
        void DrawPrefabView(RowPropPlacer tgt)
        {
            ...
            if (totalPrefabCount > 0)
            {
                ...
                for (int i = 0; i < totalPrefabCount; i++)
                {
                    ...
                    if (scrollArea.Contains(curEvent.mousePosition) && btnArea.Contains( curEvent.mousePosition ))
                    {
                        tgt.SetPreviewMeshIndex( i );
                        NeedPreviewMesh = true;
                    }
