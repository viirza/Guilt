...
namespace HyTeKGames.SRPMaterialConverter
{
    ...
    public class SRPMC_Editor : EditorWindow
    {
        ...
        void CreateGUI()
        {
            ...
            var visualTree = AssetDatabase.LoadAssetAtPath<VisualTreeAsset>(Path.Combine(GetHyTeKGamesFolderPath(), "Resources/UI Documents/SRPMC_Window.uxml"));
        ...
        async void StartConvert()
        {
            ...
            if (!Directory.Exists(Path.Combine(GetHyTeKGamesFolderPath(), "ConvertedMaterials")))
                Directory.CreateDirectory(Path.Combine(GetHyTeKGamesFolderPath(), "ConvertedMaterials"));
                    ...
                    if (parsedMat != null)
                    {
                        ...
                        if (shader)
                        {
                            ...
                            if (!overrideOriginalMaterials.value)
                            {
                                ...
                                AssetDatabase.CreateAsset(converted, Path.Combine(GetHyTeKGamesFolderPath(), "ConvertedMaterials/" + mat.name + ".mat"));
                                Material _mat = AssetDatabase.LoadAssetAtPath<Material>(Path.Combine(GetHyTeKGamesFolderPath(), "ConvertedMaterials/" + mat.name + ".mat"));
        ...
        {
            if (!Directory.Exists(Path.Combine(GetHyTeKGamesFolderPath(), "ConvertedMaterials/Maps")))
                Directory.CreateDirectory(Path.Combine(GetHyTeKGamesFolderPath(), "ConvertedMaterials/Maps"));
                    ...
                    if (evt.type == EventType.DragPerform)
                    {
                        ...
                        foreach (Object draggedObject in DragAndDrop.objectReferences)
                        {
                            ...
                            else if (draggedObject is DefaultAsset asset) // Check if the dragged object is a folder
                            {
                                ...
                                if (Directory.Exists(path)) // Verify that it's a directory
                                {
                                    // Get all material files in the directory
