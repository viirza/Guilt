...
// This file is intended for you to extend NiloToon Environment shader with your own custom logic.
...
// You can extend NiloToon Environment shader by writing additional code here without worrying about merge conflict in future updates, 
...
// You can use empty functions below to apply your global effect, similar to environment-only postprocess (e.g. add fog of war/dithered transparency/scan line/...).
...
void ApplyCustomUserLogicToVertexShaderOutputAtVertexShaderEnd(inout Varyings output, Attributes attribute)
{
	...
	//output.positionCS.xy += sin(_Time + output.positionWS.z) * 0.25; // example code, make environment vertex move in clip space
}

void ApplyCustomUserLogicToSurfaceData(inout SurfaceData surfaceData, Varyings input)
{
	// edit SurfaceData by your custom logic here

	//surfaceData.albedo *= half4(1,0,0,1); // example code, tint environment's albedo with red color
}

void ApplyCustomUserLogicBeforeFog(inout half4 color, SurfaceData surfaceData, InputData inputData)
{
    // edit color by your custom logic here

    //color.rgb *= frac(length(inputData.positionWS)); // example code, tint some pattern to environment's color before applying fog
}

void ApplyCustomUserLogicAfterFog(inout half4 color, SurfaceData surfaceData, InputData inputData)
{
    // edit color by your custom logic here

    //color *= half4(0,1,0,1); // example code, tint environment's final display pixels with green color
}
